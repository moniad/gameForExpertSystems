...
move_player_left :- not(crawls(player)), position(player, X, Y), S is X - 1, step_player(S, Y).
move_player_left :- handle_crawling(player), position(player, X, Y), S is X - 1, step_player(S, Y).

move_player_right :- not(crawls(player)), position(player, X, Y), S is X + 1, step_player(S, Y).
move_player_right :- handle_crawling(player), position(player, X, Y), S is X + 1, step_player(S, Y).

move_player_up :- not(crawls(player)), position(player, X, Y), S is Y + 1, step_player(X, S).
move_player_up :- handle_crawling(player), position(player, X, Y), S is Y + 1, step_player(X, S).

move_player_down :- not(crawls(player)), position(player, X, Y), S is Y - 1, step_player(X, S).
...
make_crawling_forbidden :- retractall(crawling_forbidden(_)), asserta(crawling_forbidden(1)), make_stand.
...
reset_crawling_counter :- retractall(players_crawling_counter(_)), asserta(players_crawling_counter(0)), make_crawling_allowed.
