...
step_opponent(X, Y) :- players_movement_counter(1), field_on_board(X, Y), retract(position(opponent, _, _)), asserta(position(opponent, X, Y)).
...
update_players_movement_counter :- players_movement_counter(1), retract(players_movement_counter(1)), asserta(players_movement_counter(0)).